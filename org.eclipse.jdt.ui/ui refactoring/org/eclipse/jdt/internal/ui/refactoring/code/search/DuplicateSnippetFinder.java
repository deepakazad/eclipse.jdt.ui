...
/* package */ class DuplicateSnippetFinder extends GenericVisitor {

	public static class Match {
		private List<ASTNode> fNodes;
		private Map<IBinding, SimpleName> fLocalMappings;
		public Match() {
			fNodes= new ArrayList<ASTNode>(10);
			fLocalMappings= new HashMap<IBinding, SimpleName>();
		}
		...
		public void addLocal(IBinding org, SimpleName local) {
			fLocalMappings.put(org, local);
		}
		public SimpleName getMappedName(IBinding org) {
		...
		public IMethodBinding getMappedBinding(IMethodBinding org) {
			SimpleName name= fLocalMappings.get(org);
			return ASTNodes.getMethodBinding(name);
		}
	...
	private class Matcher extends ASTMatcher {
		...
		public boolean match(SimpleName candidate, Object s) {
			...
			if (cb == null || sb == null)
				return false;
			//TODO: the code should be simplified and then tested thoroughly
			if (candidate.isDeclaration() && cb instanceof IMethodBinding && sb instanceof IMethodBinding) {
				SimpleName mapped= fMatch.getMappedName(sb);
				if (mapped != null) {
					IMethodBinding mappedBinding= ASTNodes.getMethodBinding(mapped);
					if (!Bindings.equals(cb, mappedBinding))
						return false;
				}
	...
	private DuplicateSnippetFinder(ASTNode[] snippet) {
		super(true);
		fSnippet= snippet;
	...
	public static Match[] perform(ASTNode start, ASTNode[] snippet) {
		...
		DuplicateSnippetFinder finder= new DuplicateSnippetFinder(snippet);
		start.accept(finder);
